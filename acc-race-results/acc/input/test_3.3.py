...
def find_project_root(start_path, target_dir_name="acc"):
    # Iteracja w górę w hierarchii katalogów, aż znajdziesz katalog o nazwie target_dir_name
    current_path = start_path
    while True:
        if os.path.basename(current_path) == target_dir_name:
            return current_path
        parent_path = os.path.dirname(current_path)
        if parent_path == current_path:  # Jeśli dotarliśmy do katalogu głównego
...
output_dir = os.path.join(project_root, "output", "Week League")
background_image = os.path.join(project_root, "pliki", "background", "background_race-results.jpg")
font_path = os.path.join(project_root, "pliki", "fonts", "BigShouldersDisplay-Bold.ttf")
