...
def convert_time(ms):
    # Dodanie poprawki do konwersji milisekund, aby prawidłowo uwzględniać zaokrąglenia
    minutes = int(ms // 60000)
    seconds = int((ms % 60000) // 1000)
    milliseconds = int(ms % 1000)
    # Konwersja czasu z uwzględnieniem pełnych 3 cyfr milisekund
...
for i, line in enumerate(data['sessionResult']['leaderBoardLines']):
    ...
    if lap_count < first_driver_laps:
        ...
        # Zmiana logiki dla mniejszej liczby okrążeń, aby poprawnie wyświetlała "+1 okrążenie"
...
for json_file in glob.glob(os.path.join(input_dir, "*.json")):
    try:
        ...
        results = []
        best_lap_times = []
        first_driver_time = None  # Zmienna do przechowywania czasu pierwszego kierowcy
        first_driver_laps = max_lap_count  # Zakładamy, że pierwszy kierowca przejechał najwięcej okrążeń
